...
```sh
mvn clean -pl maven,maven/module-info-compiler-maven-plugin,memory,memory/memory-core,memory/memory-netty,memory/memory-unsafe,format,vector package -Dsiren.arrow.enable_unsafe_memory_access=false -Dsiren.drill.enable_unsafe_memory_access=false
```
